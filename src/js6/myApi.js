...
function addclassname(ele, jiuname, xinname) {
  var str = 'xinname'; // var reg  =/(\s|^)'+jiuname+'(\s|$)/;      //是错的 ，字面量形式创建是禁止存在变量的
...
function move(ele, json, fn) {
  //先关闭定时器
  clearInterval(ele.timer); //开启定时器

  ele.timer = setInterval(function () {
    //设置一个开关为的是多个属性传入时，一个属性达到目标值时，另一个属性没有到时，不关闭定时器
    var ostop = true; //遍历对象里的所有属性值
    ...
    for (attr in json) {
      ...
      var speed = (json[attr] - icur) / 8; //调整speed

      speed = speed > 0 ? Math.ceil(speed) : Math.floor(speed); //判定这个开关是开启还是关闭 判断终止条件 (如果所有属性运动不完，把bStop = false);
